...
question_1_p <- 0.05
question_1_n <- 100
question_1_lambda <- question_1_p * question_1_n

#Calculate the poisson approximation of 0 successes
poission_approximation <- dpois(0, lambda = question_1_lambda)
poission_approximation

#Calculate the normal approximation of 0 successes
normal_approximation <- dbinom(0, question_1_n, question_1_p)
...
#Calculate the poisson approximation of 6 successes
poission_approximation <- ppois(5, lambda = question_1_lambda)
poission_approximation

#Calculate the normal approximation of 6 successes
normal_approximation <- pbinom(5, question_1_n, question_1_p)
normal_approximation
```
...
question1_dataframe <- data.frame(xVal = factor(c(0:10)), pois = dpois(c(0:10), lambda = question_1_lambda), binom = dbinom(c(0:10), question_1_n, question_1_p))
   
question1_poisboxplot <- ggplot(question1_dataframe) +
   geom_bar(stat = "identity", aes(x = xVal, y = pois, fill = xVal), color = "black") +
   ylim(0, 0.2) +
   scale_fill_brewer(palette = "PiYG") +
   labs(x = "Outcomes", y = "Binomial Probability", fill = "X Values", title = "Poisson Distribution")

question1_binomboxplot <- ggplot(question1_dataframe) +
   geom_bar(stat = "identity", aes(x = xVal, y = binom, fill = xVal), color = "black") +
   ylim(0, 0.2) +
   scale_fill_brewer(palette = "PiYG") +
   labs(x = "Outcomes", y = "Binomial Probability", fill = "X Values", title = "Binomial Distribution")

grid.arrange(question1_poisboxplot, question1_binomboxplot, ncol = 2)
...
```{r test1ci}
question1_outcomes <- c(0:6)
question1_probabilities <- c(0.215, 0.230, 0.240, 0.182, 0.130, 0.003, 0.001)
question1_discretedistribution <- data.frame(question1_outcomes, question1_probabilities)

#Calculate the expected value
question1_ev <- sum(question1_outcomes*question1_probabilities)
round(question1_ev, digits = 2)

#Calculate the variance
question1_var <- sum(((question1_outcomes-question1_ev)**2)*question1_probabilities)
round(question1_var, digits = 2)
```
...
```{r test1cii}
ggplot(question1_discretedistribution) +
   geom_bar(stat = "identity", aes(x = factor(question1_outcomes), y = cumsum(question1_probabilities), fill = factor(question1_outcomes)), color = "black") +
   geom_vline(xintercept = median(question1_outcomes) + 1, color = "darkblue") +
   scale_fill_brewer(palette = "Blues") +
   labs(x = "Outcome", y = "Cumulative Probability", fill = "Outcomes", title = "Cumulative Probability vs. Outcome")
...
ggplot(faithful, aes(x = waiting, color = waiting)) + 
   ...
   labs(x = "Wait Time (Minutes)", y = "Frequency", title = "Histogram of Waiting Times", fill = "X Values")

#Compute the empirical conditional probability
question2 <- faithful[(faithful$eruptions < 3.0) & (faithful$waiting > 70),]
empirical_probability_2a <- nrow(question2)/nrow(faithful)
empirical_probability_2a
...
```{r test2ai}
question2_faithful <- faithful
question2_faithful$criteria <- ifelse((faithful$eruptions < 3.0) & (faithful$waiting > 70), "Match", "No Match")

ggplot(question2_faithful, aes(x = waiting, y = eruptions, color = criteria)) +
   geom_rect(aes(xmin = 70, xmax = Inf, ymin = -Inf, ymax =3), alpha = 0.5) +
   geom_point() +
   geom_vline(xintercept = 70, color = "#E69F00") +
   geom_hline(yintercept = 3.0, color = "#E69F00") +
   scale_color_brewer(palette = "Dark2") +
   labs(x = "Wait Time (Minutes)", y = "Eruptions", color = "Criteria", title = "Wait Times vs. Eruptions")
...
***Answer: The above scatterplot shows a positive correlation between the eruption time and waiting time.  There are two major clusters in this plot, with Cluster 1 showing short wait times with a low amount of eruptions.  Cluster 2 shows long wait times with a high amount of eruptions.  ***
...
```{r test2b}
question2_waitingtimes <- matrix(faithful$waiting, ncol = 2, byrow = TRUE)
question2_waitingtimes

#Tried making this in ggplot but kept getting tracebacks
plot(x = question2_waitingtimes[,1], y = question2_waitingtimes[,2], xlab = "First Waiting Time", ylab = "Second Waiting Time")
...
question2_kendallcorrelation <- cor.test(question2_waitingtimes[,1],question2_waitingtimes[,2],alternative = "two.sided",method = "kendall", conf.level = 0.95)

question2_kendallcorrelation
...
question3_result1 <- result[result$Diet == 1,]
question3_result3 <- result[result$Diet == 3,]

par(mfrow = c(1,2))

#Boxplot of Diet 1
boxplot(question3_result1$weight, col = "green", ylim = c(100,400), main = "Diet 1", ylab = "Weight")

#Boxplot of Diet 3
boxplot(question3_result3$weight, col = "blue", ylim = c(100,400), main = "Diet 3", ylab = "Weight")
...
```{r test3b}
t.test(question3_result1$weight, question3_result3$weight, alternative = c("two.sided"), mu = 0, paired = FALSE, var.equal = TRUE, conf.level = .95)
...
```{r test3c}
question3_paired <- as.data.frame(cbind(pre,post))

#Create a scatterplot of volume
ggplot(data = question3_paired, aes(y = post, x = pre)) +
   geom_point(color = "#56B4E9") +
   geom_abline(intercept = 0, color = "#999999") +
   theme(legend.position = "none") +
   labs(x = "Day 20", y = "Day 21", title = "Matched Pair Chick Weight Observations")
...
alpha <- .05
n <- nrow(question3_paired)
df <- n - 1

#Calculate Test Statistic
criticalvalue <- qt(alpha, df, lower.tail = TRUE)
criticalvalue

d <- question3_paired$pre - question3_paired$post
question3_mean <- mean(d)
question3_mean

question3_sd <- sd(d)
question3_sd

teststatistic <- (question3_mean)/(question3_sd/sqrt(n))
teststatistic

#Calculate Confidence Interval
upperCI <- question3_mean - criticalvalue * (question3_sd / sqrt(n))
upperCI

lowerCI <- question3_mean + criticalvalue * (question3_sd / sqrt(n))
lowerCI

#Calculate P Value
pvalue <- pt(teststatistic, df, lower.tail = TRUE)
pvalue
...
print(paste("Skewness is", skewness(Nile)))
print(paste("Kurtosis is", kurtosis(Nile)))

par(mfrow = c(1,2))

#Create a qq-norm plot
qqnorm(Nile, col = "Green", cex = 0.5, pch = 16, main = "QQ-Plot of Nile River Flow")

#Create a boxplot
boxplot(Nile, col = "Blue", main = "Boxplot of Nile River Flow", ylab = "Annual Flow")
...
```{r test4b}
question4_nile <- c(1:length(Nile))
...
getSampleStats <- function(iterations, sampleSize, seed) {
  ...
  #Loops through 1000 samples
  for (i in 1:iterations) {
      question4_sample <- sample(question4_nile, sampleSize)
      question4_df <- data.frame(mean = mean(question4_sample))
      question4_df$i <- i
      question4_list[[i]] <- question4_df
  }
  return(question4_list)
}

#Call function to get list of stats of sample size 16, seed 124
question4_sample1 <- getSampleStats(1000, 16, 124)
sample1 = do.call(rbind, question4_sample1)

#Call function to get list of stats of sample size 64, seed 127
question4_sample2 <- getSampleStats(1000, 64, 127)
sample2 = do.call(rbind, question4_sample2)
...
#Output table
kable(rbind(sample1stats, sample2stats), format = "html", caption = "Sample Statistics of Nile Data") %>% kable_styling()
...
ggplot(data = warpbreaks, aes(x = breaks)) +
   ...
   geom_vline(xintercept = median_breaks, color = "black") +
   theme(legend.position = "none") +
   labs(x = "Number of Breaks", y = "Frequency of Breaks", lab = "Number of Breaks Histogram")
...
question5_ct <- table(warpbreaks$tension, warpbreaks$number)
question5_ct
...
```{r test5b}
question5_chisquare <- chisq.test(question5_ct)
question5_chisquare

print(paste("With a p-value of", question5_chisquare$p.value, "at the 5% level, the hypothesis should be rejected."))
...
question5_chisquare_c <- chisqfun(question5_ct)
question5_chisquare_c$p.value
question5_chisquare_c$statistic
